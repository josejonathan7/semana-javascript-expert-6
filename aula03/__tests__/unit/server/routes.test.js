...
describe("#Routes - test suite for api response", () => {
	...
	it("->GET /stream", async () => {
		const params = TestUtil.defaultHandlerParams();
		const mockPassStream = new PassThrough();

		params.request.method = "GET";
		params.request.url = "/home/stream";

		jest.spyOn(
			Controller.prototype,
			Controller.prototype.createClientStream.name
		).mockReturnValue({
			stream: mockPassStream,
			onClose: () =>{return;}
		});

		jest.spyOn(mockPassStream, "pipe").mockResolvedValue();

		await handler(...params.values());

		expect(Controller.prototype.createClientStream).toBeCalled();
		expect(mockPassStream.pipe).toHaveBeenCalledWith(params.response);

	});

	it.only("->POST /controller", async () => {
		const params = TestUtil.defaultHandlerParams();
		const order = {
			command : "start"
		};
		...
		params.request.url = "/controller";
		params.request.data = JSON.stringify(order);
		...
		expect(Controller.prototype.handleCommand).toBeCalledWith(order);
		expect(c).toEqual(1);
